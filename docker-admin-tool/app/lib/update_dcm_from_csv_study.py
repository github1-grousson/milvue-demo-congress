...
def update_dicom_file(file_path, csv_data):
    ...
    # Create/update the private tag (1351,0010) with the fixed value "Reports".
    # Update the private tag (1351,1001) with the ReportTemplateName from CSV.
    report_template_name = csv_row.get('ReportTemplateName', '')
    if report_template_name != '':
        private_tag = (0x1351, 0x1001)
        current_private_value = ''
        if private_tag in ds:
            current_private_value = str(ds.get(private_tag).value)
        if current_private_value != report_template_name:
            # Set or update the private tag with VR 'LO' (Long String).
            ds.add_new(private_tag, 'LO', report_template_name)
            updated = True
            updated_tags.append("ReportTemplateName (private)")
        private_tag_reports = (0x1351, 0x0010)
        current_reports_value = ''
        if private_tag_reports in ds:
            current_reports_value = str(ds.get(private_tag_reports).value)
        if current_reports_value != "Report_Templates":
            ds.add_new(private_tag_reports, 'LO', "Report_Templates")
            updated = True
...
def update_dicom_tags_in_dir(input_dir, csv_data):
    ...
    """
    updated_files = 0
    skip_extensions = {'.json','.csv', '.log', '.py', '.txt', '.docx', '.jpeg', '.jpg', '.png', '.pdf', '.zip', '.tar', '.gz', '.tgz', '.bz2', '.7z'}
...
def process_update_dcm(input_dir, csv_file):
...
def main():
    ...
    process_update_dcm(input_dir, csv_file)
